...
**Implements:** LOCK-03 (Tool Engine Philosophy).
...
**Implements:** LOCK-05 (Plugin-Ready Architecture), LOCK-08 (SEO Constitution), LOCK-12 (Feature Lifecycle).
...
**Implements:** LOCK-02 (Browser-First Philosophy).
...
**Implements:** LOCK-06 (Database Optional Philosophy).
...
**Implements:** LOCK-02 (browser-first), performance goals in `00_Project_Charter` §5.2.
...
No architectural element exists without a doc reference. Code without doc precedent is a violation. *Why P5:* AI-assisted development (LOCK-09) requires explicit context; undocumented decisions get duplicated or contradicted.
...
- Depends on `00_Project_Charter` (Architectural Locks) — source of authority.
...
- `10_FBRD` defines the manifest schema referenced in AD-03.
- `11_ACD` defines the Tool Engine component referenced in AD-02.
- `13_DatabaseDesign` implements the Infrastructure Layer for persistence.
- `14_APIConvention` implements the Infrastructure Layer for external APIs.
- `17_RBAC` implements the authorization cross-cutting concern.
- `21_DeploymentGuide` implements AD-06 deployment topology.
...
- `06_FolderStructure` — Granular file conventions for the layers defined here.
- `10_FBRD` §Tool Manifest — Schema referenced by AD-03.
- `11_ACD` §Tool Engine Component — Implementation of AD-02.
- `13_DatabaseDesign` — Infrastructure Layer persistence implementation.
- `14_APIConvention` — Infrastructure Layer external API implementation.
- `15_SEOSpecification` — How AD-03 registry satisfies LOCK-08.
- `16_UserFlow` — How AD-05 graceful degradation affects user journeys.
- `17_RBAC` — Authorization enforcement points referenced in §9.2.
- `18_AdminSpecification` — How admin interacts with Registry and Infrastructure.
- `21_DeploymentGuide` — Operationalizes AD-06.
- `22_AI_Guideline` — Constrains AI's interaction with this architecture (LOCK-09).
