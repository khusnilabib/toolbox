// src/tools/pdf/pdf-split/manifest.ts
...
const title = 'PDF Splitter';
const description =
  'Split a PDF by page range and save the selected pages as a new document — all in your browser.';
...
export const manifest: ToolManifest = {
  ...
  lifecycle: 'stable',
  version: '1.0.0',
  purpose: 'Extract a range of pages from a PDF into a new PDF document.',
  ...
  successCriteria: 'A new PDF containing the requested pages is produced and downloadable.',
  ...
  seo: {
    ...
    title: 'PDF Splitter — Split PDF by Pages Online Free',
    description:
      'Split a PDF by page range and save selected pages as a new document. Runs locally in your browser. No upload.',
    ...
    faq: makeFaq([
      ['Is my PDF uploaded to a server?', 'No. The PDF is split entirely in your browser.'],
      ['How do I specify which pages to keep?', 'Use a range like "1-3, 5, 8-10" (1-based, inclusive).'],
      ['Can I split into multiple files at once?', 'This tool extracts one range into a single file. Run it again for additional ranges.'],
    ]),
    breadcrumb: buildToolBreadcrumb(category, slug, title),
  },
  relatedTools: ['pdf-merge', 'pdf-extract-pages', 'pdf-rotate', 'pdf-compress'],
  analytics: {
    events: [{ name: 'pdf_split_completed', trigger: 'after processing' }],
