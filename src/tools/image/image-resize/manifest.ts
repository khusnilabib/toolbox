...
export const inputSchema = z.object({
  ...
  scale: z.number().min(1).max(100).optional(),
...
const description =
  'Resize images to exact pixel dimensions or by percentage — all in your browser, with no upload required.';
...
export const manifest: ToolManifest = {
  ...
  lifecycle: 'stable',
  version: '1.0.0',
  purpose: 'Resize an image to exact pixel dimensions or by a percentage scale factor.',
  userProblem:
    'I need to resize an image to specific dimensions or scale it down without uploading it to a server.',
  inputSchema,
  outputSchema,
  validationRules: [
  ...
  successCriteria: 'A new image file is produced with the requested dimensions and downloadable.',
  ...
  seo: {
    ...
    description:
      'Resize PNG, JPG, WebP, and GIF images to exact pixel dimensions or by percentage. Runs locally in your browser. No upload, no account.',
    ...
    faq: makeFaq([
      ...
      ['What is the maximum file size?', '25 MB per image.'],
    ]),
  ...
  analytics: {
    events: [{ name: 'resize_completed', trigger: 'after processing' }],
